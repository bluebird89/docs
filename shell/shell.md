...
* 设置 Shell 的默认执行环境参数：set 命令可以用来修改 Shell 环境的运行参数，不带任何参数的 set 命令，会显示所有的环境变量和 Shell 函数
  ...
  - command || exit 1 命令执行失败需停止：对于默认的 Shell 脚本运行环境，有运行失败的命令（返回值非0）， bash 会继续执行后面的命令
