...
* 计算机中程序关于某数据集合上的一次执行过程，包括了动态创建、调度和消亡的整个过程，是系统资源分配的最小单位
* 资源管理
  - 包括内存资源、IO资源、信号处理等部分
...
* 进程间通信 IPC InterProcess Communication
  - 很不方便：多进程地址空间不同，数据不能共享，一个进程内创建的变量在另一个进程是无法访问
  ...
  - 套接字 Socket 既可以在本机进程间通信，也可以跨网络通过
    + 通过网络接口将数据发送到本机的不同进程或远程计算机的进程
...
* 通常由线程ID、当前指令指针(PC）、寄存器集合和堆栈组成，是进程中的一个实体。线程的实体包括程序、数据和线程控制块（Thread Control Block，TCB），TCB包括以下信息：
  - 线程状态
    + 就绪
    + 阻塞
    + 运行
