...
* 每个节点最多有两个「分叉」，即两个子节点，分别是左子节点和右子节点，并不要求每个节点都有两个子节点
    - 满二叉树
...
* 概念
    ...
    - 节点的度：节点拥有的子节点数目
    - 树的度：树内各节点度的最大值
    - 节点高度（自下向上):节点到叶子节点最长路径（边数）
    - 树高度:根节点高度
    - 节点深度（自上向下):根节点到这个节点经历边个数
    - 节点层：节点深度+1
...
* 遍历
    - 前序遍历(上下左右):从根节点开始，先遍历左子树，再遍历右子树
    - 中序遍历（左右上下）:从左子树最左侧的节点->从左到右依次遍历左子树，真正的根节点->右子树
    - 后序遍历（左右下上）:从左子树最左侧的节点开始，先遍历完叶子节点，再遍历父节点，遍历完左子树后，直接从右子树最左侧节点开始，按照和左子树同样的顺序遍历完右子树，最后访问根节点
* 插入
...
* 树上的每一个结点都遵循下面的规则（提醒:这里的自平衡和平衡二叉树AVL的高度平衡有别） 三黑不同
    ...
    - 树的根结点为黑色
    - 树中不存在两个相邻红色结点（即红色结点的父结点和孩子结点均不能是红色）
