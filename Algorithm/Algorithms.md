...
* bubble sort 冒泡
    ...
    - 外层控制执行次数n：i o~n
    - 内层控制比较范围：j从0开始到n-i-1,与后一个值俩俩比较（比较次数 n-1 n-2 1）
    - 时间复杂度:O(n2)
    - 空间复杂度：只涉及相邻元素的交换，是原地排序算法
    - 算法稳定性：元素相等不会交换，是稳定的排序算法
* Insert Sort **插入**
    - 将数组中数据分为已排序区间和未排序区间两个区间，初始已排序区间只有一个元素，就是数组第一个元素
    - **取未排序区间中元素，在已排序区间中插入合适位置**，并保证已排序区间数据有序。重复这个过程，直到未排序区间中元素为空
    - 外层i从1到n（未排序区间），0 为已排序区间,i 取出 value
    - 内层 j 从i-1到0 与value比较，a[j] > valuie a[j]后移一位
        + 比value大后移一位，小于等于value赋值到j+1
    ...
    - 时间复杂度是O(n2)
* Selection Sort 选择
    - 分已排序区间和未排序区间。每次会**从未排序区间中选择最小元素，放到已排序区间末尾**
    - 对比插入：排序空间不一样
        + 选择：未排序中最小
        + 插入：插入到已排序中合适位置
        + 冒泡：**未排序中找到最大，放到已排序头部**
