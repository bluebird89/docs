...
* binlog
  ...
  - 格式
    + STATMENT：基于 SQL 语句复制( statement-based replication, SBR )，每一条修改数据sql语句会记录到 binlog 中
      * 优点：不需要记录每一行变化，减少了 binlog 日志量，节约了 IO , 从而提高性能
      * 缺点：在某些情况下会导致主从数据不一致，比如执行sysdate() 、 slepp() 等
    + ROW：基于行复制(row-based replication, RBR )，不记录每条sql语句上下文信息，仅需记录哪条数据被修改了
  ...
  - binlog最大序号 pow(2,31)-1 = 2147483647
  - 隐患
...
- redo log
  + InnoDB 引擎提供的日志系统，最开始 MySQL 里并没有 InnoDB 引擎。MySQL 自带引擎是 MyISAM，但是 MyISAM 没有 crash-safe 的能力，binlog 日志只能用于归档，不具备数据库崩溃重启后数据恢复功能
  + InnoDB 是另一个公司以插件形式引入 MySQL 的，因为只依靠 binlog 是没有 crash-safe 能力，所以 InnoDB 使用另外一套日志系统——也就是 redo log 来实现 crash-safe 能力
...
- undo log：原子性是指对数据库的一系列操作，要么全部成功，要么全部失败，不可能出现部分成功情况
...
* 类型
  - 共享锁 Shared Locks（S锁，MyISAM 叫做读锁）：由读表操作加上的锁，加锁后其他用户只能获取该表或行的共享锁，不能获取排它锁，也就是说只能读不能写 `select ... lock in share mode`
  - 排它锁 Exclusive Locks（X锁，MyISAM 叫做写锁）：由写表操作加上的锁，加锁后其他用户不能获取该表或行的任何锁，允许事务删除或更新一行数据 `select ... for update`
...
* 意向锁（表级别锁）
  - InnoDB存储引擎还支持多粒度锁定，允许事务在行级上的锁和表级上的锁同时存在
...
+ InnoDB 行锁算法
  ...
  - Gap lock 间隙锁：在记录之间加锁，或者在第一个记录之前加锁，亦或者在最后一个记录之后加锁，即锁定一个范围，而非记录本身
    + 目的:为了阻止多个事务将记录插入到同一范围内，而这会导致幻读问题的产生
    + 显式关闭gap锁：（除了外键约束和唯一性检查外，其余情况仅使用record lock）
      * 将事务隔离级别设置为RC
      * 将参数innodb_locks_unsafe_for_binlog设置为1
  - Next-key lock：record+gap 锁定一个范围，包含记录本身
    + Innodb对于行查询使用next-key lock
...
* LOCK TABLES 和 UNLOCK TABLES:MySQL Server层实现
...
* 定义：一颗多路平衡查找树，一颗M阶B-树
  - 阶数：表示了一个节点最多有多少个孩子节点
  - 每个节点都存有关键字与子树的指针
    + 关键字:键值对,对应的key和value
    + 关键字按照值从小到大升序排序
    + 关键字的左子树中的所有关键字都小于它，右子树中的所有关键字都大于它
    + 定义  m/2 <= k ≤ m
  - 每个节点关键字 至少M/2-1（取上整）和至多M-1个（至少2个关键字）
  - 根结点子节点数：[2, M] 关键字：1 <= k <= m-1
  - 非根节点关键字：[M/2,M-1]
  - 中间节点（除根结点以外的非叶子结点）的子节点为[M/2, M]  k-1 个关键字和 k 个子结点
...
* 在MongoDB的索引设计上就使用了B树做索引
  - 在MongoDB这样的NoSLQ数据库中，在数据表的设计层面就决定了不会有太多的遍历和范围查找，基本就是一个键对应一个值的单一查询
  - 由于每个节点都可以存放数据，幸运的话有可能不必找到叶子节点就能取得数据
...
* 为什么 MySQL 的索引使用 B+ 树而不是 B 树呢
  ...
  - B+确实能够减少大量的磁盘IO，从而大大提高查询性能，而且由于B+树根节点的数据已经是排序好的双向链表，我们可以像链表一样遍历所有数据，非常适合范围查找和排序操作
* 参考
...
* 优点：提升数据检索效率
  ...
  - 将随机 I/O 变为顺序 I/O
* 缺点
  - 创建和维护要耗费资源，并且随着数据量的增加耗费资源也会增加
  - 占据空间
  - 增加、删除、修改时，索引需要动态维护，降低了数据维护速度
* PRIMARY
  - 创建
...
* FULLTEXT
  - 只在MYISAM 存储引擎支持,目的是全文索引，在VARCHAR或者TEXT类型的列上创建，在内容系统中用的多， 在全英文网站用多(英文词独立). 中文数据不常用，意义不大 国内全文索引通常使用 sphinx 来完成.全文索引:fulltext是Myisam表特殊索引，从文本中找关键字不是直接和索引中的值进行比较
  - MySQL 自带全文索引只能用于 MyISAM ，并且只能对英文进行全文检索
* 索引选择性
...
* 联合索引：两个或更多个列上索引，可以包括15个列
  - 构建：以索引 (name, city, gender) 为例，首先是按照 name 字段顺序组织，当 name 字段的值相同时（如 Bush），其按照 city 字段顺序组织，当 city 字段值相同时，其按照 gender 字段组织
...
* 水平拆分
  - 以字段为依据，按照一定策略（hash、range等），将一个库（表）中的数据拆分到多个库（表）中。分库内分表和分库两部分，每片数据会分散到不同的MySQL表或库，达到分布式的效果，能够支持非常大的数据量。前面的表分区本质上也是一种特殊的库内分表
    + 库内分表，仅仅是单纯的解决了单一表数据过大的问题，由于没有把表的数据分布到不同的机器上，因此对于减轻MySQL服务器的压力来说，并没有太大的作用，还是竞争同一个物理机上的IO、CPU、网络，这个就要通过分库来解决
    + 分库分表
  - 每个库（表）结构一样,数据都不一样，没有交集
  - 所有库（表）并集是全量数据
  - 场景
    + 存在单库读写、存储性能瓶颈，系统绝对并发量并没有上来，只是单表的数据量太多，影响了SQL效率，加重了CPU负担，以至于成为瓶颈 => 表的数据量少了，单次SQL执行效率高，自然减轻了CPU的负担
  ...
  - 水平分库
    + 把同一个表按一定规则拆分到不同的数据库中，每个库可以位于不同的服务器上，以此实现水平扩展
  ...
  - 通过userId后四位mod 32分到32个库中，同时再将userId后四位div 32 mod 32将每个库分为32个表，共计分为1024张表。线上部署情况为8个集群(主从)，每个集群4个库
