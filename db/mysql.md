s# [MySQL Server](https://github.com/mysql/mysql-server)
...
* binlog
  ...
  - 模式
    + STATMENT：基于 SQL 语句复制( statement-based replication, SBR )，记录可能引起数据变更的 sql 语句
...
* undo log 回滚日志 记录数据被修改前的信息
  ...
  - 为了在发生错误时回滚之前的操作，需要将之前的操作都记录下来，然后在发生错误时才可以回滚。
  - 对原子性支持：原子性是指对数据库的一系列操作，要么全部成功，要么全部失败，不可能出现部分成功情况
  - 用来回滚数据的用于保障 未提交事务的原子性
  + MVCC(多版本并发控制)实现关键
...
+ InnoDB 行锁算法
  ...
  - Gap Locks 间隙锁：在记录之间加锁，或者在第一个记录之前加锁，亦或者在最后一个记录之后加锁，即锁定一个范围，而非记录本身
    + 目的:为了阻止多个事务将记录插入到同一范围内，而导致幻读问题的产生
...
* 默认的锁粒度行级锁 row level locking，并发性能比较好，会发生死锁的情况
...
* 多版本并发控制 MultiVersion Concurrency Control MVCC 来支持高并发
  - InnoDB的 MVCC 通过在每行记录的后面保存两个隐藏的列来实现的。这两个列，一个保存了行的创建时间，一个保存了行的过期时间，当然存储的并不是实际的时间值，而是系统版本号
...
* 索引结构
  - 聚集索引的叶节点包含了完整的数据记录，辅助索引叶节点数据部分是主键的值，除了空间索引外，InnoDB的索引实现基本都是 B+ 树
