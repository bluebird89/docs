...
* 为服务容器中可用类提供了一个「静态」接口.是服务容器中底层类的静态代理，提供了简洁而富有表现力的语法，甚至比传统的静态方法更具可测试性和扩展性
* 原理
  - Facade 提前注入 Ioc 容器
...
* Facades Vs 依赖注入
  ...
  - Facades 使用动态方法来代理从服务容器解析的对象的方法调用，可以像测试注入的类实例一样来测试 Facades
* 辅助函数 底层实际是调用facade
* 可被 mock 的。使用 shouldReceive 方法 mock 对静态外观方法的调用，该方法将返回 Mockery mock 的实例
